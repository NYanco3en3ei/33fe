...
const initializeMockData = () => {
  try {
    ...
    if (!localStorage.getItem('salespersons')) {
      ...
      localStorage.setItem('salespersons', JSON.stringify(mockSalespersons));
    }
  } catch (error) {
...
export default function App() {
  ...
  // 添加页面加载完成后的日志记录，便于调试部署问题
  useEffect(() => {
    console.log('应用已成功加载，当前环境:', process.env.NODE_ENV);
  }, []);
